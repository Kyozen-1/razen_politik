...
class RelawanDptManualImpor implements ToCollection,WithStartRow
{
    protected $dapil_id;
        ...
        try{
            ...
            foreach ($rows as $kunci=>$row) {
                ...
                if ($row->filter()->isNotEmpty()) {
                    ...
                    if($row[13]==null){
                        $response['import_status'] = false;
                        $response['import_message'] = 'Status Pemilih Harap Diisi';
                        session(['import_status' => $response['import_status']]);
                        session(['import_message' => $response['import_message']]);
                        return false;
                    }

                    if($row[13] != 'ya' && $row[13] != 'tidak'){
                        $response['import_status'] = false;
                        $response['import_message'] = 'Status Pemilih harus ya / tidak';
                        session(['import_status' => $response['import_status']]);
                        session(['import_message' => $response['import_message']]);
                        return false;
                    }
                        ...
                        if($cek_kabupaten)
                        {
                            ...
                            if($cek_kecamatan)
                            {
                                ...
                                if($cek_kelurahan)
                                {
                                    ...
                                    $dpt_manual->pengguna_id = Auth::user()->relawan->koordinator->pengguna_id;
