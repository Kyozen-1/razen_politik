...
<div class="data-table-rows slim">
    ...
    <div class="data-table-responsive-wrapper">
        <table id="saksi_table" class="data-table w-100">
            <thead>
                <tr>
                    <th>No</th>
                    <th>Nama</th>
    ...
    <div class="modal-dialog modal-dialog-centered modal-xl">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form class="form-horizontal" id="saksi_form" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
...
@section('js')
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            var dataTables = $('#saksi_table').DataTable({
                processing: true,
                serverSide: true,
                ajax:{
                    url:"{{route('pengguna.saksi.index')}}"
                },
                columns:[
                    {
                        data: 'DT_RowIndex',
        ...
        $('#saksi_form').on('submit', function(e){
            e.preventDefault();
            if($('#aksi').val() == 'Save')
            {
                $.ajax({
                    url: "{{ route('pengguna.saksi.store') }}",
                    method: "POST",
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.success)
                        {
                            ...
                            $('#saksi_form')[0].reset();
                            $('#aksi_button').text('Save');
                            $('#saksi_table').DataTable().ajax.reload();
                        }
            ...
            {
                $.ajax({
                    url: "{{ route('pengguna.saksi.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            var url = "{{ route('pengguna.saksi.show', ['id' => ":id"]) }}";
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    ...
                    $('#detail_foto').attr('src', "{{asset('images/pengguna/saksi')}}" + '/' + data.result.foto);
                    $('#detail').modal('show');
                }
            });
        });
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            $('#form_result').html('');
            var url = "{{ route('pengguna.saksi.edit', ['id' => ":id"]) }}";
            url = url.replace(':id', id);
            $('#form_email').hide();
            $.ajax({
                url: url,
                ...
                success: function(data)
                {
                    ...
                    var lokasi_foto = "{{ asset('images/pengguna/saksi') }}"+'/'+data.result.foto;
        ...
        $('#ok_button').click(function(){
            var url = "{{ route('pengguna.saksi.destroy', ['id' => ":user_id"]) }}";
