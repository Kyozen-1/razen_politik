...
@section('title', 'Jadwal Kunjungan Relawan | Koordinator')
                        ...
                        <div class="row">
                            ...
                            <div class="col-12">
                                <div class="mb-3 position-relative form-group">
                                    ...
                                    <select name="relawan_id" id="relawan_id" class="form-control" required>
                                        <option value="">--- Pilih Relawan---</option>
                                        @foreach ($relawan as $id => $nama)
                                            <option value="{{$id}}">{{$nama}}</option>
                                        @endforeach
                                    </select>
                                </div>
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            var dataTables = $('#jadwal_kunjungan_relawan_table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('koordinator.jadwal-kunjungan-relawan.index') }}"
                },
                columns: [
                    {
                        data: 'DT_RowIndex',
        ...
        $('#create').click(function(){
            ...
            $("[name='relawan_id']").val('').trigger('change');
            $('#kabupaten_kota_id').empty();
            $('#kecamatan_id').empty();
            $('#kelurahan_id').empty();
            $('#kabupaten_kota_id').prop('disabled', true);
        ...
        $('#jadwal_kunjungan_relawan_form').on('submit', function(e){
            ...
            {
                $.ajax({
                    url: "{{ route('koordinator.jadwal-kunjungan-relawan.store') }}",
                    method: "POST",
            ...
            {
                $.ajax({
                    url: "{{ route('koordinator.jadwal-kunjungan-relawan.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            var url = "{{ route('koordinator.jadwal-kunjungan-relawan.show', ['id' => ":id"]) }}";
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            $('#form_result').html('');
            var url = "{{ route('koordinator.jadwal-kunjungan-relawan.edit', ['id' => ":id"]) }}";
            url = url.replace(':id', id);
            $('#form_email').hide();
            $.ajax({
                url: url,
                ...
                success: function(data)
                {
                    ...
                    $("[name='relawan_id']").val(data.result.relawan_id).trigger('change');
        ...
        $('#ok_button').click(function(){
            var url = "{{ route('koordinator.jadwal-kunjungan-relawan.destroy', ['id' => ":user_id"]) }}";
