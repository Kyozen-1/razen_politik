...
@section('content')
        ...
        <div class="col-12 col-md-7">
            ...
            <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                <ul class="breadcrumb pt-0">
                    <li class="breadcrumb-item"><a href="{{ route('koordinator.profil.index') }}">Profil</a></li>
                </ul>
            </nav>
        </div>
        <!-- Title End -->
    ...
    <div class="card mb-3">
        <div class="card-body">
            <form action="{{ route('koordinator.profil.store') }}" novalidate="novalidate" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row mb-3">
                ...
                <div class="row">
                    <div class="col-md-8 col-12">
                        <div class="form-group position-relative mb-3">
                            ...
                            <input type="text" class="form-control" id="nama" name="nama" value="{{$koordinator->nama}}" required>
                        </div>
                        <div class="form-group position-relative mb-3">
                            <label for="no_hp" class="form-label">No HP</label>
                            <input type="text" class="form-control" id="no_hp" name="no_hp" value="{{$koordinator->no_hp}}" required>
                        </div>
                        <div class="form-group position-relative mb-3">
                            <label for="jenis_kelamin" class="form-label">Jenis Kelamin</label>
                            <select name="jenis_kelamin" id="jenis_kelamin" class="form-control" required>
                                <option value="L" @if($koordinator->jenis_kelamin == 'L') selected @endif>Laki - Laki</option>
                                <option value="P" @if($koordinator->jenis_kelamin == 'P') selected @endif>Perempuan</option>
                            </select>
                        </div>
                    </div>
                    <div class="col-md-4 col-12">
                        <div class="form-group position-relative mb-3">
                            ...
                            <input type="file" class="dropify" name="foto" id="foto" data-height="300" data-allowed-file-extensions="png jpg jpeg" data-show-errors="true" data-default-file="{{ asset('images/pengguna/koordinator/'.$koordinator->foto) }}" required>
                        </div>
                    </div>
                    <div class="col-12">
                        <div class="row">
                            ...
                            <div class="col-12">
                                <div class="mb-3 position-relative form-group">
                                    ...
                                    <textarea name="alamat" id="alamat" rows="5" class="form-control" required>{{$koordinator->alamat}}</textarea>
                                </div>
                            </div>
                            <div class="col-6">
                                <div class="mb-3 position-relative form-group">
                                    <label for="rt" class="form-label">RT</label>
                                    <input type="text" class="form-control" id="rt" name="rt" placeholder="ex: 001" value="{{$koordinator->rt}}" required>
                                </div>
                            </div>
                            <div class="col-6">
                                <div class="mb-3 position-relative form-group">
                                    <label for="rw" class="form-label">RW</label>
                                    <input type="text" class="form-control" id="rw" name="rw" placeholder="ex: 001" value="{{$koordinator->rt}}" required>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    <div class="card mb-3">
        <div class="card-body">
            ...
            @endif
            <form action="{{ route('koordinator.profil.ganti-password') }}" novalidate="novalidate" method="POST" enctype="multipart/form-data">
                @csrf
...
@section('js')
    ...
    <script>
        var state_kabupaten_kota_id = "{{$koordinator->kabupaten_kota_id}}";
        var state_kecamatan_id = "{{$koordinator->kecamatan_id}}";
        var state_kelurahan_id = "{{$koordinator->kelurahan_id}}";
        ...
        $(document).ready(function(){
            ...
            $("[name='provinsi_id']").val("{{$koordinator->provinsi_id}}").trigger('change');
        });
        ...
        $('#provinsi_id').on('change', function(){
            ...
            {
                $.ajax({
                    url: "{{ route('koordinator.get-data.kabupaten') }}",
                    method: 'POST',
                    data: {
                        "_token": "{{ csrf_token() }}",
                        id:$(this).val()
        ...
        $('#kabupaten_kota_id').on('change', function(){
            ...
            {
                $.ajax({
                    url: "{{ route('koordinator.get-data.kecamatan') }}",
                    method: 'POST',
                    data: {
                        "_token": "{{ csrf_token() }}",
                        id:$(this).val()
        ...
        $('#kecamatan_id').on('change', function(){
            ...
            {
                $.ajax({
                    url: "{{ route('koordinator.get-data.kelurahan') }}",
                    method: 'POST',
                    data: {
                        "_token": "{{ csrf_token() }}",
                        id:$(this).val()
