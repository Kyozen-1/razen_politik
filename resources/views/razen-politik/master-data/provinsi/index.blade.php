...
@section('title', 'Razen Politik | Master | Provinsi')
...
@section('content')
    ...
    <div class="page-title-container">
        ...
        <div class="col-12 col-md-7">
            <h1 class="mb-0 pb-0 display-4" id="title">Provinsi</h1>
            <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                <ul class="breadcrumb pt-0">
                    <li class="breadcrumb-item"><a href="#">Master Data</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('razen-politik.master-data.provinsi.index') }}">Provinsi</a></li>
                </ul>
            </nav>
        </div>
        <!-- Title End -->
    ...
    <div class="row mb-3">
        <div class="col-12" style="text-align:right">
            ...
            <a href="{{ asset('template/template_impor_provinsi.xlsx') }}" class="btn btn-outline-secondary waves-effect waves-light" title="File Template"><i class="fas fa-file-excel"></i></a>
    ...
    <div class="data-table-rows slim">
        ...
        <div class="data-table-responsive-wrapper">
            <table id="master_provinsi_table" class="data-table w-100">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Kode</th>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form class="form-horizontal" id="master_provinsi_form" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-3 position-relative form-group">
                            <label for="kode" class="control-label">Kode Provinsi</label>
                            <input type="text" class="form-control" name="kode" id="kode" required>
                        </div>
                        <div class="mb-3 position-relative form-group">
                            <label for="nama" class="control-label">Nama Provinsi</label>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group mb-3 position-relative">
                        <label for="detail_kode" class="control-label">Kode Provinsi</label>
                        <input type="text" id="detail_kode" class="form-control" disabled>
                    </div>
                    <div class="form-group mb-3 position-relative">
    ...
    <div id="imporModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="addEditModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <span id="form_result"></span>
                    <form class="form-horizontal" action="{{ route('razen-politik.master-data.provinsi.impor') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-3 position-relative form-group">
...
@section('js')
    ...
    <script>
        $(document).ready(function(){
            ...
            var dataTables = $('#master_provinsi_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('razen-politik.master-data.provinsi.index') }}"
                },
                columns: [
                    {
                        data: 'DT_RowIndex',
        ...
        $('#master_provinsi_form').on('submit', function(e){
            e.preventDefault();
            if($('#aksi').val() == 'Save')
            {
                $.ajax({
                    url: "{{ route('razen-politik.master-data.provinsi.store') }}",
                    method: "POST",
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.success)
                        {
                            ...
                            $('#master_provinsi_form')[0].reset();
                            $('#aksi_button').text('Save');
                            $('#master_provinsi_table').DataTable().ajax.reload();
                        }
            ...
            {
                $.ajax({
                    url: "{{ route('razen-politik.master-data.provinsi.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            var url = "{{ route('razen-politik.master-data.provinsi.show', ['id' => ":id"]) }}";
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            $('#form_result').html('');
            var url = "{{ route('razen-politik.master-data.provinsi.edit', ['id' => ":id"]) }}";
            url = url.replace(':id', id);
            $.ajax({
                url: url,
                dataType: "json",
        ...
        $('#ok_button').click(function(){
            var url = "{{ route('razen-politik.master-data.provinsi.destroy', ['id' => ":user_id"]) }}";
