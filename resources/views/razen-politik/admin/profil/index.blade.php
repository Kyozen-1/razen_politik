...
@section('title', 'Razen Politik | Profil')
...
@section('content')
    ...
    <div class="card mb-3">
        <div class="card-body">
            <form action="{{ route('razen-politik.admin.profil.store') }}" novalidate="novalidate" method="POST" enctype="multipart/form-data">
                @csrf
                <div class="row mb-3">
                ...
                <div class="row">
                    ...
                    <div class="col-12 col-md-4">
                        <div class="mb-3 position-relative form-group">
                            ...
                            <input type="file" class="dropify" name="logo" data-height="300" data-allowed-file-extensions="png jpg jpeg webp" data-default-file="{{ asset('images/razen-politik/logo/'.$profil->logo) }}" data-show-errors="true" required>
                        </div>
                        <div class="mb-3 position-relative form-group">
                            <label for="logo_kecil" class="form-label">Logo Kecil</label>
                            <input type="file" name="logo_kecil" class="dropify" data-height="300" data-allowed-file-extensions="png jpg jpeg webp" data-default-file="{{ asset('images/razen-politik/logo/'.$profil->logo_kecil) }}" data-show-errors="true" required>
                        </div>
                    </div>
                </div>
            </form>
    ...
    <script>
        ...
        $('#edit_media_sosial_profil_form').submit(function(e){
            ...
            return new swal({
                ...
                {
                    $.ajax({
                        url: "{{ route('razen-politik.admin.profil.edit-media-sosial-profil') }}",
                        method: "POST",
                        data: {
                            "_token": "{{ csrf_token() }}",
                            ...
                            if(data.success)
                            {
                                new swal({
                                    ...
                                    title: data.success
                                        ...
                                        window.location.href = "{{ route('razen-politik.admin.profil.index') }}";
                                });
                            }
                        }
                    });
        ...
        $('#tambah_media_sosial_profil_form').submit(function(e){
            e.preventDefault();
            $.ajax({
                url: "{{ route('razen-politik.admin.profil.tambah-media-sosial-profil') }}",
                method: "POST",
