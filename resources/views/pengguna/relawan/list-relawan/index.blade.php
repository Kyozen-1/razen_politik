...
@section('title', 'List Relawan | Master Data | Pengguna')
...
<div class="page-title-container">
    ...
    <div class="col-12 col-md-7">
        <h1 class="mb-0 pb-0 display-4" id="title">List Relawan | Master Data</h1>
...
<div class="data-table-rows slim">
    ...
    <div class="data-table-responsive-wrapper">
        <table id="list_relawan_table" class="data-table w-100">
            <thead>
                <tr>
                    <th>No</th>
                    <th>Nama</th>
    ...
    <div class="modal-dialog modal-dialog-centered modal-xl">
        <div class="modal-content">
            ...
            <div class="modal-body">
                ...
                <form class="form-horizontal" id="list_relawan_form" method="POST" enctype="multipart/form-data">
                    @csrf
                    <div class="row">
                        ...
                        <div class="row">
                            <div class="col-12">
                                <div class="form-group position-relative mb-3">
                                    <label for="detail_nama_koordinator" class="form-label">Koordinator</label>
...
@section('js')
    ...
    <script>
        ...
        $(document).ready(function(){
            ...
            var dataTables = $('#list_relawan_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('pengguna.relawan.list-relawan.index') }}"
                },
                columns: [
                    {
                        data: 'DT_RowIndex',
        ...
        $('#create').click(function(){
            ...
            $('#list_relawan_form')[0].reset();
            $("[name='koordinator_id']").val('').trigger('change');
        ...
        $('#list_relawan_form').on('submit', function(e){
            e.preventDefault();
            if($('#aksi').val() == 'Save')
            {
                $.ajax({
                    url: "{{ route('pengguna.relawan.list-relawan.store') }}",
                    method: "POST",
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.success)
                        {
                            ...
                            $('#list_relawan_form')[0].reset();
                            $('#aksi_button').text('Save');
                            $('#list_relawan_table').DataTable().ajax.reload();
                        }
            ...
            {
                $.ajax({
                    url: "{{ route('pengguna.relawan.list-relawan.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            var url = "{{ route('pengguna.relawan.list-relawan.show', ['id' => ":id"]) }}";
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    ...
                    $('#detail_foto').attr('src', "{{asset('images/pengguna/relawan')}}" + '/' + data.result.foto);
                    $('#detail').modal('show');
                }
            });
        });
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            $('#form_result').html('');
            var url = "{{ route('pengguna.relawan.list-relawan.edit', ['id' => ":id"]) }}";
            url = url.replace(':id', id);
            $('#form_email').hide();
            $.ajax({
                url: url,
                ...
                success: function(data)
                {
                    ...
                    var lokasi_foto = "{{ asset('images/pengguna/relawan') }}"+'/'+data.result.foto;
        ...
        $('#ok_button').click(function(){
            var url = "{{ route('pengguna.relawan.list-relawan.destroy', ['id' => ":user_id"]) }}";
