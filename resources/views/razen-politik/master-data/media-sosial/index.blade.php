        ...
        <div class="col-12 col-md-7">
            ...
            <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                <ul class="breadcrumb pt-0">
                    ...
                    <li class="breadcrumb-item"><a href="{{ route('razen-politik.master-data.media-sosial.index') }}">Master Media Sosial</a></li>
                </ul>
            </nav>
        </div>
        <!-- Title End -->
    ...
    <script>
        $(document).ready(function(){
            ...
            var dataTables = $('#media_sosial_table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('razen-politik.master-data.media-sosial.index') }}"
                },
                columns: [
                    {
                        data: 'DT_RowIndex',
        ...
        $('#master_media_sosial_form').on('submit', function(e){
            ...
            {
                $.ajax({
                    url: "{{ route('razen-politik.master-data.media-sosial.store') }}",
                    method: "POST",
            ...
            {
                $.ajax({
                    url: "{{ route('razen-politik.master-data.media-sosial.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            var url = "{{ route('razen-politik.master-data.media-sosial.show', ['id' => ":id"]) }}";
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            $('#form_result').html('');
            var url = "{{ route('razen-politik.master-data.media-sosial.edit', ['id' => ":id"]) }}";
            url = url.replace(':id', id);
            $.ajax({
                url: url,
                dataType: "json",
        ...
        $('#ok_button').click(function(){
            var url = "{{ route('razen-politik.master-data.media-sosial.destroy', ['id' => ":id"]) }}";
