...
use App\Imports\KoordinatorDataPemilihImpor;
...
class DataPemilihController extends Controller
{
    public function index()
    {
        if(request()->ajax())
        {
            $data = DptManual::where('pengguna_id', Auth::user()->koordinator->pengguna_id);
        ...
        return view('koordinator.data-pemilih.index', [
            'provinsi' => $provinsi
        ]);
    }
    ...
    public function impor(Request $request)
    {
        ...
        // import data
        Excel::import(new KoordinatorDataPemilihImpor($dapil_id), $file);

        $msg = [session('import_status'), session('import_message')];

        if ($msg[0]) {
            Alert::success('Berhasil', $msg[1]);
            return redirect()->route('pengguna.data-pemilih.index');
        } else {
