...
@section('title', 'Razen Politik | Master | Kabupaten / Kota')
...
@section('content')
    ...
    <div class="page-title-container">
        ...
        <div class="col-12 col-md-7">
            <h1 class="mb-0 pb-0 display-4" id="title">Kabupaten / Kota</h1>
            <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                <ul class="breadcrumb pt-0">
                    <li class="breadcrumb-item"><a href="#">Master Data</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('razen-politik.master-data.kabupaten-kota.index') }}">Kabupaten / Kota</a></li>
                </ul>
            </nav>
        </div>
        <!-- Title End -->
    ...
    <div class="row mb-3">
        <div class="col-12" style="text-align:right">
            ...
            <a href="{{ asset('template/template_impor_kabupaten.xlsx') }}" class="btn btn-outline-secondary waves-effect waves-light" title="File Template"><i class="fas fa-file-excel"></i></a>
    ...
    <div class="data-table-rows slim">
        ...
        <div class="data-table-responsive-wrapper">
            <table id="master_kabupate_kota_table" class="data-table w-100">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Provinsi</th>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form class="form-horizontal" id="master_kabupaten_kota_form" method="POST" enctype="multipart/form-data">
                        @csrf
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group mb-3 position-relative">
                        <label for="detail_kode" class="form-label">Kode Kabupaten</label>
    ...
    <div id="imporModal" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="addEditModalLabel" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <span id="form_result"></span>
                    <form class="form-horizontal" action="{{ route('razen-politik.master-data.kabupaten-kota.impor') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-3 position-relative form-group">
...
@section('js')
    ...
    <script>
        $(document).ready(function(){
            ...
            var dataTables = $('#master_kabupate_kota_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('razen-politik.master-data.kabupaten-kota.index') }}"
                },
                columns: [
                    {
                        data: 'DT_RowIndex',
        ...
        $('#create').click(function(){
            ...
            $('#master_kabupaten_kota_form')[0].reset();
            $('.dropify-clear').click();
        ...
        $('#master_kabupaten_kota_form').on('submit', function(e){
            e.preventDefault();
            if($('#aksi').val() == 'Save')
            {
                $.ajax({
                    url: "{{ route('razen-politik.master-data.kabupaten-kota.store') }}",
                    method: "POST",
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.success)
                        {
                            ...
                            $('#master_kabupaten_kota_form')[0].reset();
                            $('#aksi_button').text('Save');
                            $('#master_kabupate_kota_table').DataTable().ajax.reload();
                        }
            ...
            {
                $.ajax({
                    url: "{{ route('razen-politik.master-data.kabupaten-kota.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            var url = "{{ route('razen-politik.master-data.kabupaten-kota.show', ['id' => ":id"]) }}";
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            $('#form_result').html('');
            var url = "{{ route('razen-politik.master-data.kabupaten-kota.edit', ['id' => ":id"]) }}";
            url = url.replace(':id', id);
            $.ajax({
                url: url,
                dataType: "json",
        ...
        $('#ok_button').click(function(){
            var url = "{{ route('razen-politik.master-data.kabupaten-kota.destroy', ['id' => ":user_id"]) }}";
