...
@section('title', 'Pemilu | Admin | Razen Politik')
...
@section('content')
    ...
    <div class="page-title-container">
        ...
        <div class="col-12 col-md-7">
            <h1 class="mb-0 pb-0 display-4" id="title">Pemilu</h1>
            <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                <ul class="breadcrumb pt-0">
                    <li class="breadcrumb-item"><a href="#">Admin</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('razen-politik.admin.pemilu.index') }}">Pemilu</a></li>
                </ul>
            </nav>
        </div>
        <!-- Title End -->
    ...
    <div class="data-table-rows slim">
        ...
        <div class="data-table-responsive-wrapper">
            <table id="master_pemilu_table" class="data-table w-100">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Nama</th>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form class="form-horizontal" id="pemilu_form" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="mb-3 position-relative form-group">
                            ...
                            <input type="date" class="form-control" name="tgl_pemilihan" id="tgl_pemilihan" required>
                        </div>
                </div>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group mb-3 position-relative">
                        <label for="detail_tgl_pemilihan" class="form-label">Tanggal Pemilihan</label>
                        <input type="text" id="detail_tgl_pemilihan" class="form-control" disabled>
                    </div>
                </div>
            </div><!-- /.modal-content -->
        </div><!-- /.modal-dialog -->
...
@section('js')
    ...
    <script>
        $(document).ready(function(){
            ...
            var dataTables = $('#master_pemilu_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('razen-politik.admin.pemilu.index') }}"
                },
                columns: [
                    {
                        data: 'DT_RowIndex',
        ...
        $('#pemilu_form').on('submit', function(e){
            e.preventDefault();
            if($('#aksi').val() == 'Save')
            {
                $.ajax({
                    url: "{{ route('razen-politik.admin.pemilu.store') }}",
                    method: "POST",
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.success)
                        {
                            ...
                            $('#pemilu_form')[0].reset();
                            $('#aksi_button').text('Save');
                            $('#master_pemilu_table').DataTable().ajax.reload();
                        }
            ...
            {
                $.ajax({
                    url: "{{ route('razen-politik.admin.pemilu.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            var url = "{{ route('razen-politik.admin.pemilu.show', ['id' => ":id"]) }}";
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    ...
                    $('#detail_tgl_pemilihan').val(data.result.tgl_pemilihan);
                    $('#detail').modal('show');
                }
            });
        });
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            $('#form_result').html('');
            var url = "{{ route('razen-politik.admin.pemilu.edit', ['id' => ":id"]) }}";
            url = url.replace(':id', id);
            $.ajax({
                url: url,
                dataType: "json",
                success: function(data)
                {
                    ...
                    $('#tgl_pemilihan').val(data.result.tgl_pemilihan);
        ...
        $('#ok_button').click(function(){
            var url = "{{ route('razen-politik.admin.pemilu.destroy', ['id' => ":user_id"]) }}";
