...
@section('title', 'Paket | Master Data | Admin | Razen Politik')
...
@section('content')
    ...
    <div class="data-table-rows slim">
        ...
        <div class="data-table-responsive-wrapper">
            <table id="master_paket_table" class="data-table w-100">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Nama</th>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form class="form-horizontal" id="master_paket_form" method="POST" enctype="multipart/form-data">
                        @csrf
...
@section('js')
    ...
    <script>
        $(document).ready(function(){
            ...
            var dataTables = $('#master_paket_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('razen-politik.master-data.paket.index') }}"
                },
                columns: [
                    {
                        data: 'DT_RowIndex',
        ...
        $('#master_paket_form').on('submit', function(e){
            e.preventDefault();
            if($('#aksi').val() == 'Save')
            {
                $.ajax({
                    url: "{{ route('razen-politik.master-data.paket.store') }}",
                    method: "POST",
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.success)
                        {
                            ...
                            $('#master_paket_form')[0].reset();
                            $('#aksi_button').text('Save');
                            $('#master_paket_table').DataTable().ajax.reload();
                        }
            ...
            {
                $.ajax({
                    url: "{{ route('razen-politik.master-data.paket.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            var url = "{{ route('razen-politik.master-data.paket.show', ['id' => ":id"]) }}";
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            $('#form_result').html('');
            var url = "{{ route('razen-politik.master-data.paket.edit', ['id' => ":id"]) }}";
            url = url.replace(':id', id);
            $.ajax({
                url: url,
                dataType: "json",
        ...
        $('#ok_button').click(function(){
            var url = "{{ route('razen-politik.master-data.paket.destroy', ['id' => ":user_id"]) }}";
