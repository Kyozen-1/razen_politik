...
<script>
    const settings = new Settings({
        @if (Auth::guard('relawan')->check())
            attributes: {
                color: "{{Auth::guard('relawan')->user()->color_layout?Auth::guard('relawan')->user()->color_layout:'light-blue'}}",
                navcolor: "{{Auth::guard('relawan')->user()->nav_color?Auth::guard('relawan')->user()->nav_color:'light'}}",
                behaviour: "{{Auth::guard('relawan')->user()->behaviour?Auth::guard('relawan')->user()->behaviour:'pinned'}}",
                layout: "{{Auth::guard('relawan')->user()->layout?Auth::guard('relawan')->user()->layout:'fluid'}}",
                radius: "{{Auth::guard('relawan')->user()->radius?Auth::guard('relawan')->user()->radius:'flat'}}",
                placement: "{{Auth::guard('relawan')->user()->placement?Auth::guard('relawan')->user()->placement:'vertical'}}",
            }
        @endif
    });
    $('.option').click(function(){
        ...
        $.ajax({
            url: "{{ route('relawan.dashboard.change') }}",
            method: 'POST',
            data: {
                "_token": "{{ csrf_token() }}",
