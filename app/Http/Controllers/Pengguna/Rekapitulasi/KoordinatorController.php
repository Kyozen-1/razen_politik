...
class KoordinatorController extends Controller
{
    public function index()
    {
        if(request()->ajax())
        {
            $data = MasterProvinsi::whereHas('koordinator', function($q){
                        $q->where('pengguna_id', Auth::user()->pengguna_id);
                    });
            $data = $data->withCount([
                'koordinator as laki_laki' => function($q) {
                    $q->where('pengguna_id', Auth::user()->pengguna_id);
                    $q->where('jenis_kelamin', 'L');
                },
                'koordinator as perempuan' => function($q) {
                    $q->where('pengguna_id', Auth::user()->pengguna_id);
                    $q->where('jenis_kelamin', 'P');
                },
                'koordinator as total' => function($q) {
                    $q->where('pengguna_id', Auth::user()->pengguna_id);
                }
            ]);
