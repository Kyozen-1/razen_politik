...
<script>
    const settings = new Settings({
        @if (Auth::guard('pengguna')->check())
            attributes: {
                color: "{{Auth::guard('pengguna')->user()->color_layout?Auth::guard('pengguna')->user()->color_layout:'light-blue'}}",
                navcolor: "{{Auth::guard('pengguna')->user()->nav_color?Auth::guard('pengguna')->user()->nav_color:'light'}}",
                behaviour: "{{Auth::guard('pengguna')->user()->behaviour?Auth::guard('pengguna')->user()->behaviour:'pinned'}}",
                layout: "{{Auth::guard('pengguna')->user()->layout?Auth::guard('pengguna')->user()->layout:'fluid'}}",
                radius: "{{Auth::guard('pengguna')->user()->radius?Auth::guard('pengguna')->user()->radius:'flat'}}",
                placement: "{{Auth::guard('pengguna')->user()->placement?Auth::guard('pengguna')->user()->placement:'vertical'}}",
            }
        @endif
    });
    $('.option').click(function(){
        ...
        $.ajax({
            url: "{{ route('pengguna.dashboard.change') }}",
            method: 'POST',
            data: {
                "_token": "{{ csrf_token() }}",
