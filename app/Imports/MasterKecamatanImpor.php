...
class MasterKecamatanImpor implements ToCollection,WithStartRow
{
                ...
                if ($row->filter()->isNotEmpty()) {
                    ...
                    if($cek_provinsi)
                    {
                        ...
                        if($cek_kabupaten)
                        {
                            ...
                            if(!$cek_kecamatan)
                            {
                                $kabupaten = new MasterKecamatan;
                                $kabupaten->kabupaten_kota_id = $cek_kabupaten->id;
                                $kabupaten->kode = $row[3];
                                $kabupaten->nama = $row[4];
                                $kabupaten->save();
                            ...
                            $response['import_status'] = false;
                            $response['import_message'] = 'Kode Kabupaten Pada Kecamatan '.$row[4].' Tidak Ditemukan';
                            session(['import_status' => $response['import_status']]);
                            session(['import_message' => $response['import_message']]);
                            return false;
                        }
                        ...
                        $response['import_status'] = false;
                        $response['import_message'] = 'Kode Provinsi Pada Kecamatan '.$row[4].' Tidak Ditemukan';
                        session(['import_status' => $response['import_status']]);
                        session(['import_message' => $response['import_message']]);
                        return false;
                    }
