...
@section('title', 'Data DPT | Master Data | Admin | Razen Politik')
...
@section('content')
    ...
    <div class="page-title-container">
        ...
        <div class="col-12 col-md-7">
            <h1 class="mb-0 pb-0 display-4" id="title">DPT</h1>
            <nav class="breadcrumb-container d-inline-block" aria-label="breadcrumb">
                <ul class="breadcrumb pt-0">
                    <li class="breadcrumb-item"><a href="#">Master Data</a></li>
                    <li class="breadcrumb-item"><a href="{{ route('razen-politik.master-data.dpt.index') }}">DPT</a></li>
                </ul>
            </nav>
        </div>
        <!-- Title End -->
    ...
    <div class="data-table-rows slim">
        ...
        <div class="data-table-responsive-wrapper">
            <table id="master_dpt_table" class="data-table w-100">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Provinsi</th>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form class="form-horizontal" id="master_dpt_form" method="POST" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="mb-3 position-relative form-group">
                            ...
                            <input type="number" class="form-control" name="jumlah_lk" id="jumlah_lk" required>
                        </div>
                        <div class="mb-3 position-relative form-group">
                            <label for="jumlah_p" class="form-label">Jumlah Perempuan</label>
                            <input type="number" class="form-control" name="jumlah_p" id="jumlah_p" required>
                        </div>
                </div>
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group mb-3 position-relative">
                        <label for="detail_nama_provinsi" class="form-label">Pronvisi</label>
                        <input type="text" id="detail_nama_provinsi" class="form-control" disabled>
                    </div>
                    <div class="form-group mb-3 position-relative">
                        <label for="detail_jumlah_lk" class="form-label">Jumlah Laki - Laki</label>
                        <input type="text" id="detail_jumlah_lk" class="form-control" disabled>
                    </div>
                    <div class="form-group mb-3 position-relative">
                        <label for="detail_jumlah_p" class="form-label">Jumlah Perempuan</label>
                        <input type="text" id="detail_jumlah_p" class="form-control" disabled>
                    </div>
                </div>
            </div><!-- /.modal-content -->
        </div><!-- /.modal-dialog -->
...
@section('js')
    ...
    <script>
        $(document).ready(function(){
            ...
            var dataTables = $('#master_dpt_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('razen-politik.master-data.dpt.index') }}"
                },
                columns: [
                    {
                        data: 'DT_RowIndex',
        ...
        $('#master_dpt_form').on('submit', function(e){
            e.preventDefault();
            if($('#aksi').val() == 'Save')
            {
                $.ajax({
                    url: "{{ route('razen-politik.master-data.dpt.store') }}",
                    method: "POST",
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.success)
                        {
                            ...
                            $('#master_dpt_form')[0].reset();
                            $('#aksi_button').text('Save');
                            $('#master_dpt_table').DataTable().ajax.reload();
                        }
            ...
            {
                $.ajax({
                    url: "{{ route('razen-politik.master-data.dpt.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            var url = "{{ route('razen-politik.master-data.dpt.show', ['id' => ":id"]) }}";
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    ...
                    $('#detail_nama_provinsi').val(data.result.nama_provinsi);
                    $('#detail_jumlah_lk').val(data.result.jumlah_lk);
                    $('#detail_jumlah_p').val(data.result.jumlah_p);
                    $('#detail').modal('show');
                }
            });
        });
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            $('#form_result').html('');
            var url = "{{ route('razen-politik.master-data.dpt.edit', ['id' => ":id"]) }}";
            url = url.replace(':id', id);
            $.ajax({
                url: url,
                dataType: "json",
                success: function(data)
                {
                    ...
                    $('#jumlah_lk').val(data.result.jumlah_lk);
                    $('#jumlah_p').val(data.result.jumlah_p);
                    $('#hidden_id').val(id);
        ...
        $('#ok_button').click(function(){
            var url = "{{ route('razen-politik.master-data.dpt.destroy', ['id' => ":user_id"]) }}";
