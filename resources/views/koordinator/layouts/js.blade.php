...
<script>
    const settings = new Settings({
        @if (Auth::guard('koordinator')->check())
            attributes: {
                color: "{{Auth::guard('koordinator')->user()->color_layout?Auth::guard('koordinator')->user()->color_layout:'light-blue'}}",
                navcolor: "{{Auth::guard('koordinator')->user()->nav_color?Auth::guard('koordinator')->user()->nav_color:'light'}}",
                behaviour: "{{Auth::guard('koordinator')->user()->behaviour?Auth::guard('koordinator')->user()->behaviour:'pinned'}}",
                layout: "{{Auth::guard('koordinator')->user()->layout?Auth::guard('koordinator')->user()->layout:'fluid'}}",
                radius: "{{Auth::guard('koordinator')->user()->radius?Auth::guard('koordinator')->user()->radius:'flat'}}",
                placement: "{{Auth::guard('koordinator')->user()->placement?Auth::guard('koordinator')->user()->placement:'vertical'}}",
            }
        @endif
    });
    $('.option').click(function(){
        ...
        $.ajax({
            url: "{{ route('koordinator.dashboard.change') }}",
            method: 'POST',
            data: {
                "_token": "{{ csrf_token() }}",
