...
@section('title', 'Fitur | Master Data | Admin | Razen Politik')
...
@section('content')
    ...
    <div class="page-title-container">
        ...
        <div class="col-12 col-md-7">
            <h1 class="mb-0 pb-0 display-4" id="title">Fitur</h1>
    ...
    <div class="data-table-rows slim">
        ...
        <div class="data-table-responsive-wrapper">
            <table id="master_fitur_table" class="data-table w-100">
                <thead>
                    <tr>
                        <th>No</th>
                        <th>Role</th>
        ...
        <div class="modal-dialog modal-dialog-centered modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    ...
                    <form class="form-horizontal" id="master_fitur_form" method="POST" enctype="multipart/form-data">
                        @csrf
                        <div class="mb-3 position-relative form-group">
                            <label for="role_id" class="form-label">Role</label>
                            <select name="role_id" id="role_id" class="form-control" required>
                                <option value="">--- Pilih Role ---</option>
                                @foreach ($role as $id => $nama)
            ...
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <div class="form-group mb-3 position-relative">
                        <label for="detail_role" class="form-label">Role</label>
                        <input type="text" id="detail_role" class="form-control" disabled>
                    </div>
                    <div class="form-group mb-3 position-relative">
...
@section('js')
    ...
    <script>
        $(document).ready(function(){
            ...
            var dataTables = $('#master_fitur_table').DataTable({
                processing: true,
                serverSide: true,
                ajax: {
                    url: "{{ route('razen-politik.master-data.fitur.index') }}"
                },
                columns: [
                    {
                        data: 'DT_RowIndex',
        ...
        $('#master_fitur_form').on('submit', function(e){
            e.preventDefault();
            if($('#aksi').val() == 'Save')
            {
                $.ajax({
                    url: "{{ route('razen-politik.master-data.fitur.store') }}",
                    method: "POST",
                    ...
                    success: function(data)
                    {
                        ...
                        if(data.success)
                        {
                            ...
                            $("[name='role_id']").val('').trigger('change');
                            $('#aksi_button').text('Save');
                            $('#master_fitur_table').DataTable().ajax.reload();
                        }
            ...
            {
                $.ajax({
                    url: "{{ route('razen-politik.master-data.fitur.update') }}",
                    method: "POST",
                    data: new FormData(this),
                    dataType: "json",
                    contentType: false,
        ...
        $(document.body).on('click', '.detail', function(){
            var id = $(this).attr('id');
            var url = "{{ route('razen-politik.master-data.fitur.show', ['id' => ":id"]) }}";
            ...
            $.ajax({
                ...
                success: function(data)
                {
                    ...
                    $('#detail_role').val(data.result.nama_role);
                    $('#detail').modal('show');
                }
            });
        });
        ...
        $(document).on('click', '.edit', function(){
            var id = $(this).attr('id');
            $('#form_result').html('');
            var url = "{{ route('razen-politik.master-data.fitur.edit', ['id' => ":id"]) }}";
            url = url.replace(':id', id);
            $.ajax({
                url: url,
                dataType: "json",
                success: function(data)
                {
                    ...
                    $("[name='role_id']").val(data.result.role_id).trigger('change');
                    $('#hidden_id').val(id);
        ...
        $('#ok_button').click(function(){
            var url = "{{ route('razen-politik.master-data.fitur.destroy', ['id' => ":user_id"]) }}";
