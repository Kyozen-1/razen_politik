...
<script>
    const settings = new Settings({
        @if (Auth::guard('razen_politik')->check())
            attributes: {
                color: "{{Auth::guard('razen_politik')->user()->color_layout?Auth::guard('razen_politik')->user()->color_layout:'light-blue'}}",
                navcolor: "{{Auth::guard('razen_politik')->user()->nav_color?Auth::guard('razen_politik')->user()->nav_color:'light'}}",
                behaviour: "{{Auth::guard('razen_politik')->user()->behaviour?Auth::guard('razen_politik')->user()->behaviour:'pinned'}}",
                layout: "{{Auth::guard('razen_politik')->user()->layout?Auth::guard('razen_politik')->user()->layout:'fluid'}}",
                radius: "{{Auth::guard('razen_politik')->user()->radius?Auth::guard('razen_politik')->user()->radius:'flat'}}",
                placement: "{{Auth::guard('razen_politik')->user()->placement?Auth::guard('razen_politik')->user()->placement:'vertical'}}",
            }
        @endif
    });
    $('.option').click(function(){
        ...
        $.ajax({
            url: "{{ route('razen-politik.admin.dashboard.change') }}",
            method: 'POST',
            data: {
                "_token": "{{ csrf_token() }}",
